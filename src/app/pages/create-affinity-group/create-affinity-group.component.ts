...
@Component({
  selector: 'app-create-affinity-group',
  templateUrl: './create-affinity-group.component.html',
  styleUrl: './create-affinity-group.component.scss'
})
export class CreateAffinityGroupComponent {
      ...
      this.userService.genOtpPages(otpObj).subscribe(
        res => {
          if (res['error'] == 0) {
            ...
            this.showDiv.affinityDiv = false;
  ...
  otpverify() {
    ...
    if (this.userOtp) {
      ...
      this.userService.sendAffinityMail(data).subscribe({
        next: (res) => {
          this.stopOtpTimer();

          this.contactForm.reset();
          this.submitted = false;
          console.log('Response from sendAffinityMail:', res);
          this.toaster.success('Affinity mail sent successfully');
          this.otpValid = true;
          this.isValid = true;
